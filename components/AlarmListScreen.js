...
const AlarmListScreen = (props) => {
    ...
    return (
        ...
        <View style={styles.screen}>
           ...
           <Pressable style={[styles.button]} onPress={()=>{if(selectMode){deleteAlarm()}else{props.navigation.navigate('AlarmOptions', {alarm:false})}}}>
                <View style={{ flex:1}}>
                    {selectMode?<Image style={[styles.button_del, {tintColor:'white'}]} source={bin}></Image>:<Image style={[styles.button,styles.left]} source={add}></Image>}
                </View>
            </Pressable>
            <Pressable style={[styles.button_1, styles.left_1]} onPress={()=>{props.navigation.navigate('PillStatus')}}>
            <View style={{ flex:1}}>
            {selectMode?<Image></Image>:<Image style={styles.button_1} source={view}></Image>}
            </View>
        </Pressable>
        </View>
    )
...
const styles = StyleSheet.create({
    ...
    left:{
        position:'absolute',
        bottom:30,
        left: Dimensions.get('screen').width/2-(Dimensions.get('screen').width/3),
    }
    ,
    button_1:{
        width: Dimensions.get('screen').width/5,
    ...
    left_1:{
        position:'absolute',
        bottom:30,
        left: Dimensions.get('screen').width/2+(Dimensions.get('screen').width/5),
    },
    button_del:{
        width: Dimensions.get('screen').width/4,
        height: Dimensions.get('screen').width/4,
        left: Dimensions.get('screen').width/1.75-(Dimensions.get('screen').width/5),
        tintColor:'white',
        bottom:50,
        },
